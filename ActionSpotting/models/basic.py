...
class SALayer(nn.Module):
    ...
    def __init__(self, q_dim, nhead, ff_dim=2048, kv_dim=None,
        ...
        self.linear1 = nn.Linear(q_dim, ff_dim)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(ff_dim, q_dim)
    ...
    def __str__(self) -> str:
        return f"SALayer( q({self.q_dim})xkv({self.kv_dim})->{self.q_dim}, head:{self.nhead}, ffdim:{self.ff_dim}, dropout:{self.dropout_rate}, vpos:{self.use_vpos} )"
...
class SCALayer(nn.Module):
    ...
    def __init__(self, action_dim, frame_dim, nhead, ff_dim=2048, dropout=0.1, attn_dropout=0.1,
        ...
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(action_dim, ff_dim)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(ff_dim, action_dim)
        ...
        self.string = f"SCALayer( adim:{action_dim}, fdim:{frame_dim}, head:{nhead}, ffdim:{ff_dim}, dropout:{(dropout, attn_dropout)}, svpos:{sa_value_w_pos}, cvpos:{ca_value_w_pos} )"
