...
from utils.helpers import parse_label, expand_frame_label, easy_reduce, to_numpy
...
class Video():
    ...
    def __init__(self, vname: str, start_time: int, end_time: int, **kwargs):
        self.vname = vname
...
def save_results(ckpt: Checkpoint, vnames: list, label_list: list, attrs_saves: list, start_time: list, end_time: list) -> list:
    videos = []
    for i in range(len(vnames)):
        video = Video(vnames[i], start_time = start_time[i], end_time = end_time[i], gt_label=to_numpy(label_list[i]), **attrs_saves[i])
        videos.append(video)
